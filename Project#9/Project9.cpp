...
const char *WINDOWTITLE = { "Project #9 - Haoxiang Wang" };
...
const char* TEMPFORMAT = { "S Range: %5.2f - %5.2f" };
const char* OPACITYFORMAT = { "Maximum Opacity: %5.2f" };
float TempLowHigh[2];
float OpacitySliderValue[1];
GLUI_HSlider * TempSlider;
GLUI_StaticText * TempLabel;
GLUI_HSlider * OpacitySlider;
GLUI_StaticText * OpacityLabel;
...
Sliders(int id)
{
	...
	case TEMP:
		...
		break;
	case OPACITY:
		sprintf(str, OPACITYFORMAT, OpacitySliderValue[0]);
		OpacityLabel->set_text(str);
		break;
	}
...
Buttons(int id)
{
	...
	case RESET: //need to be changed more
		...
		OpacitySliderValue[0] = 0.;
		sprintf(str, OPACITYFORMAT, OpacitySliderValue[0]);
		OpacityLabel->set_text(str);
		OpacitySlider->set_float_limits(0., 1.);
...
Display(void)
{
	...
	if (Major == Y)
	{
		...
		ycoord = y0;
		for (int y = 0; y < NY; y++, ycoord += dy)
		{
			glTexImage2D(GL_TEXTURE_2D, 0, 4, NX, NZ, 0, GL_RGBA, GL_UNSIGNED_BYTE, &TextureXZ[y][0][0][0]);

			glBegin(GL_QUADS);

			// the TextureXZ array is dimensioned [NY][NZ][NX]:
			glTexCoord2f(0.f, 0.f);
			glVertex3f(-1.f, ycoord, - 1.f);
	...
	if (Major == X)
	{
		...
		xcoord = x0;
		for (int x = 0; x < NX; x++, xcoord += dx)
		{
			glTexImage2D(GL_TEXTURE_2D, 0, 4, NZ, NY, 0, GL_RGBA, GL_UNSIGNED_BYTE, &TextureYZ[x][0][0][0]);

			glBegin(GL_QUADS);

			// the TextureYZ array is dimensioned [NX][NZ][NY]:
			glTexCoord2f(0.f, 0.f);
			glVertex3f(xcoord, - 1.f, -1.f);
	...
	glLoadIdentity();
	glColor3f(1., 1., 1.);
	DoRasterString(5., 5., 0., "Project #9");
...
InitGlui(void)
{
	...
	Glui->add_separator();
	//opacity spinner
	OpacitySlider = Glui->add_slider(false, GLUI_HSLIDER_FLOAT, OpacitySliderValue, OPACITY, (GLUI_Update_CB)Sliders);
	OpacitySlider->set_float_limits(0., 1.);
	OpacitySlider->set_w(200);		// good slider width
	sprintf(str, OPACITYFORMAT, OpacitySliderValue[0]);
	OpacityLabel = Glui->add_statictext(str);
	...
	for (int zz = 0; zz < NZ; zz++)
	{
		...
		for (int y = 0; y < NY; y++)
		{
			...
			for (int x = 0; x < NX; x++)
			{
				...
				if ((Nodes[x][y][z].t < TempLowHigh[0]) || (Nodes[x][y][z].t > TempLowHigh[1]))
				{
					r = g = b = 0.;
					alpha = 0.;
				}
				else
				{
					...
					g = Nodes[x][y][z].g;
					b = Nodes[x][y][z].b;
					alpha = OpacitySliderValue[0];
				}

				// "...4-component RGBA texture" :
