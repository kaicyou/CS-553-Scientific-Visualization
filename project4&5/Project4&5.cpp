...
const char *WINDOWTITLE = { "Project #4 & #5 - Haoxiang Wang" };
...
void Sliders(int);
void ProcessColor(struct node*, struct node*, struct node*, struct node*);
void ProcessQuad(struct node*, struct node*, struct node*, struct node*,int);
void NewPlane(int);
...
int PlaneXYOn, PlaneYZOn, PlaneXZOn;
...
void ProcessColor(struct node *p0, struct node *p1, struct node *p2, struct node *p3)
{
	glColor3f(p0->rgb[0], p0->rgb[1], p0->rgb[2]);
...
void ProcessQuad(struct node *p0, struct node *p1, struct node *p2, struct node *p3, int s)
{
	int n;
	float ss[4][3];
	float tt;
	float hsv[3], rgb[3];
		...
		if (p1->t != p0->t)
		{
			...
			if ((0 <= tt)&&(tt <= 1))
			{
				ss[n][0] = p0->x + tt * (p1->x - p0->x);
				ss[n][1] = p0->y + tt * (p1->y - p0->y);
				ss[n][2] = p0->z + tt * (p1->z - p0->z);
				n++;
			}
		}
		//second edge
		if (p2->t != p1->t)
		{
			...
			if ((0 <= tt) && (tt <= 1))
			{
				ss[n][0] = p1->x + tt * (p2->x - p1->x);
				ss[n][1] = p1->y + tt * (p2->y - p1->y);
				ss[n][2] = p1->z + tt * (p2->z - p1->z);
				n++;
			}
		}
		//third edge
		if (p3->t != p2->t)
		{
			...
			if ((0 <= tt) && (tt <= 1))
			{
				ss[n][0] = p2->x + tt * (p3->x - p2->x);
				ss[n][1] = p2->y + tt * (p3->y - p2->y);
				ss[n][2] = p2->z + tt * (p3->z - p2->z);
				n++;
			}
		}
		//fourth edge
		if (p0->t != p3->t)
		{
			...
			if ((0 <= tt) && (tt <= 1))
			{
				ss[n][0] = p3->x + tt * (p0->x - p3->x);
				ss[n][1] = p3->y + tt * (p0->y - p3->y);
				ss[n][2] = p3->z + tt * (p0->z - p3->z);
				n++;
			}
		}
		...
		if ((n == 1) || (n == 3))
		{
			printf("Error! Wrong intersection number!\n");
			exit(-1);
		}
		else if (n == 2)
		{
			hsv[0] = 240. - 240. * ((s - TEMPMIN) / (TEMPMAX - TEMPMIN));
			hsv[1] = 1.;
			hsv[2] = 1.;
			HsvRgb(hsv, rgb);
			glColor3f(rgb[0], rgb[1], rgb[2]);
			glVertex3f(ss[0][0], ss[0][1], ss[0][2]);
			glVertex3f(ss[1][0], ss[1][1], ss[1][2]);
		}
}
...
Display( void )
{
	...
	if (PlaneXYOn == GLUITRUE)
	{
		for (int i = 0; i < NX; i++)
		{
			for (int j = 0; j < NY; j++)
			{
				...
				//get rgb
				hsv[0] = 240. - 240. * ((PlaneXY[i][j].t - TEMPMIN) / (TEMPMAX - TEMPMIN));
				hsv[1] = 1.;
				hsv[2] = 1.;
				HsvRgb(hsv, rgb);
				//store rgb
				PlaneXY[i][j].rgb[0] = rgb[0];
				PlaneXY[i][j].rgb[1] = rgb[1];
				PlaneXY[i][j].rgb[2] = rgb[2];
			}
		}
		if (WhichCuttingPlane == SHADING)
		{
			glBegin(GL_QUADS);
			for (int i = 0; i < NX - 1; i++)
			{
				...
				{
					ProcessColor(&PlaneXY[i][j], &PlaneXY[i + 1][j], &PlaneXY[i + 1][j + 1], &PlaneXY[i][j + 1]);
				}
			}
			glEnd();
		}
		else if (WhichCuttingPlane == CONTOURS)
		{
			glBegin(GL_LINES);
			for (int i = 0; i < NX - 1; i++)
			{
				for (int j = 0; j < NY - 1; j++)
				{
					for (int s = TEMPMIN; s <= TEMPMAX; s += (101 - NsSteps))
					{
						ProcessQuad(&PlaneXY[i][j], &PlaneXY[i + 1][j], &PlaneXY[i + 1][j + 1], &PlaneXY[i][j + 1],s);
					}
				}
			}
			glEnd();
	...
	if (PlaneYZOn == GLUITRUE)
	{
		for (int i = 0; i < NY; i++)
		{
			for (int j = 0; j < NZ; j++)
			{
				...
				//get rgb
				hsv[0] = 240. - 240. * ((PlaneYZ[i][j].t - TEMPMIN) / (TEMPMAX - TEMPMIN));
				hsv[1] = 1.;
				hsv[2] = 1.;
				HsvRgb(hsv, rgb);
				//store rgb
				PlaneYZ[i][j].rgb[0] = rgb[0];
				PlaneYZ[i][j].rgb[1] = rgb[1];
				PlaneYZ[i][j].rgb[2] = rgb[2];
			}
		}
		if (WhichCuttingPlane == SHADING)
		{
			...
			for (int i = 0; i < NY - 1; i++)
			{
				...
				{
					ProcessColor(&PlaneYZ[i][j], &PlaneYZ[i + 1][j], &PlaneYZ[i + 1][j + 1], &PlaneYZ[i][j + 1]);
				}
			}
			glEnd();
		}
		else if (WhichCuttingPlane == CONTOURS)
		{
			...
			for (int i = 0; i < NY - 1; i++)
			{
				for (int j = 0; j < NZ - 1; j++)
				{
					for (int s = TEMPMIN; s <= TEMPMAX; s += (101 - NsSteps))
					{
						ProcessQuad(&PlaneYZ[i][j], &PlaneYZ[i + 1][j], &PlaneYZ[i + 1][j + 1], &PlaneYZ[i][j + 1],s);
					}
				}
			}
			glEnd();
	...
	if (PlaneXZOn == GLUITRUE)
	{
		for (int i = 0; i < NX; i++)
		{
			for (int j = 0; j < NZ; j++)
			{
				...
				//get rgb
				hsv[0] = 240. - 240. * ((PlaneXZ[i][j].t - TEMPMIN) / (TEMPMAX - TEMPMIN));
				hsv[1] = 1.;
				hsv[2] = 1.;
				HsvRgb(hsv, rgb);
				//store rgb
				PlaneXZ[i][j].rgb[0] = rgb[0];
				PlaneXZ[i][j].rgb[1] = rgb[1];
				PlaneXZ[i][j].rgb[2] = rgb[2];
			}
		}
		if (WhichCuttingPlane == SHADING)
		{
			...
			for (int i = 0; i < NX - 1; i++)
			{
				for (int j = 0; j < NZ - 1; j++)
				{
					ProcessColor(&PlaneXZ[i][j], &PlaneXZ[i + 1][j], &PlaneXZ[i + 1][j + 1], &PlaneXZ[i][j + 1]);
				}
			}
			glEnd();
		}
		else if (WhichCuttingPlane == CONTOURS)
		{
			...
			for (int i = 0; i < NX - 1; i++)
			{
				for (int j = 0; j < NZ - 1; j++)
				{
					for (int s = TEMPMIN; s <= TEMPMAX; s += (101 - NsSteps))
					{
						ProcessQuad(&PlaneXZ[i][j], &PlaneXZ[i + 1][j], &PlaneXZ[i + 1][j + 1], &PlaneXZ[i][j + 1], s);
					}
				}
			}
			glEnd();
	...
	if (WireSurfaceOn == GLUITRUE)
	{
		...
		for (int i = 0; i < NX; i++)
		{
			for (int j = 0; j < NY - 1; j++)
			{
				...
				{
					ProcessQuad(&Nodes[i][j][k], &Nodes[i][j + 1][k], &Nodes[i][j + 1][k + 1], &Nodes[i][j][k + 1], WireSstar);
				}
			}
		}
		for (int j = 0; j < NY; j++)
		{
			for (int i = 0;	i < NX - 1; i++)
			{
				for (int k = 0; k < NZ - 1; k++)
				{
					ProcessQuad(&Nodes[i][j][k], &Nodes[i + 1][j][k], &Nodes[i + 1][j][k + 1], &Nodes[i][j][k + 1], WireSstar);
				}
			}
		}
		for (int k = 0; k < NZ; k++)
		{
			for (int i = 0; i < NX - 1; i++)
			{
				for (int j = 0; j < NY - 1; j++)
				{
					ProcessQuad(&Nodes[i][j][k], &Nodes[i + 1][j][k], &Nodes[i + 1][j + 1][k], &Nodes[i][j + 1][k], WireSstar);
				}
			}
		}
		glEnd();
	...
	glLoadIdentity( );
	glColor3f( 1., 1., 1. );
	DoRasterString( 5., 5., 0., "Project #4 & #5" );
...
InitGlui( void )
{
	...
	Glui->add_checkbox("Point Cloud", &PointCloudOn);
	...
	bigpanel = Glui->add_panel("Cutting Plane");
		...
		panel = Glui->add_panel_to_panel( bigpanel, "" );
        		spinner = Glui->add_spinner_to_panel( panel, "X Plane", GLUI_SPINNER_FLOAT, &PlaneX, 0, (GLUI_Update_CB)NewPlane );
        		spinner->set_float_limits( -1, 1 );
			spinner->set_speed( 5.0 );
        		spinner = Glui->add_spinner_to_panel( panel, "Y Plane", GLUI_SPINNER_FLOAT, &PlaneY, 0, (GLUI_Update_CB)NewPlane );
        		spinner->set_float_limits( -1, 1 );
			spinner->set_speed( 5.0 );
        		spinner = Glui->add_spinner_to_panel( panel, "Z Plane", GLUI_SPINNER_FLOAT, &PlaneZ, 0, (GLUI_Update_CB)NewPlane );
        		spinner->set_float_limits( -1, 1 );
			spinner->set_speed( 5.0 );
...
InitGraphics( void )
{
	...
	for (int i = 0; i < NX; i++)
	{
		float m = -1. + 2. * (float)i / (float)(NX - 1);
		for (int j = 0; j < NY; j++)
		{
			float n = -1. + 2. * (float)j / (float)(NY - 1);
				//calculate x, y, z, t
				PlaneXY[i][j].x = m;
				PlaneXY[i][j].y = n;
				PlaneYZ[i][j].y = m;
...
Reset( void )
{
	...
	//added into program
	PointCloudOn = GLUIFALSE;
	PlaneXYOn = GLUIFALSE;
	PlaneYZOn = GLUIFALSE;
	PlaneXZOn = GLUIFALSE;
	WhichCuttingPlane = SHADING;
	NsSteps = 5;
	PlaneX = PlaneY = PlaneZ = 0;
	WireSurfaceOn = GLUIFALSE;
}
